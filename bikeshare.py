...
def select_city():
    ...
    while state.lower() not in all_states: #set the value user type as lowercase in city variable
        ...
        if state.lower() == 'new york':
            ...
            print('Sorry we cant find that, make sure you select one state from the list [Chicago, New York, or Washington].')
...
def ask_period():
    ...
    while period.lower() not in available_period:
        period = input('\nWould you like the data by month, day,'' or not at all? Select "none" for no filter.\n')
